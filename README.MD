...
## 重要
* 请阅读下面的内容. 工具生成code，请勿手动修改

## 工具使用
> 在slp-go使用，提供rpc对外服务
```shell
> go build -o codec main
> 将 codec 执行文件移入go的bin目录中，注意权限
> codec -t blocks -h 3    生成表明为block的缓存codec，并设置缓存过期时间为3小时
> codec -t blocks -s 100    生成表明为block的缓存codec，并设置缓存过期时间为100秒
```
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMTI0MDIzNjYsLTE4MzQ0MTI5OSwtMT
UyMDg0MDYyMiw4MzM2NzU0NzhdfQ==
-->
